...
#endif
  {
    ...
    for (uint8_t line=0; line<8; line++) {
      ...
      if (failsafeEditable) {
        ...
        uint8_t len = limit((uint8_t)1, uint8_t((abs(failsafeValue) * wbar/2 + lim/2) / lim), uint8_t(wbar/2));
        uint8_t x0 = (failsafeValue>0) ? x+COL_W-ofs-3-wbar/2 : x+COL_W-ofs-2-wbar/2-len;
